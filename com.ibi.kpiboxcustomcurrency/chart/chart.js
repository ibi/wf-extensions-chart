  ...
  if(typeof window.comIbiKpiboxChartExtension == 'undefined') {
	...
	function _draw(ib3SLI, isDummyData) {
		...
		var numberFormat = ib3SLI.config.getFormatByBucketName('value', 0),
			...
			currencyPosition = ib3SLI.config.getProperty('kpiboxProperties.currencyPosition');;
		...
		var kpiDataElem = data[0],
			...
			currency = typeof kpiDataElem.currency === 'undefined' ? '' : kpiDataElem.currency,
...
*/
		...
		var kpiValueElem = infoContainer.append('text')
			...
			.attr('x', 0)
			.text((currencyPosition == 'left') ? currency+ " " + kpiFormattedValue : kpiFormattedValue + " " + currency);
