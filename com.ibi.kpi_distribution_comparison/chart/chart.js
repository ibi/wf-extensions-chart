...
//			comparison_maxColor = isDummyData ? '#5a5a5a': ib3SLI.config.getProperty('comparison_colors.maxColor'),
		...
		var defaultMin = 0,
			auxMax = 0,
			maxDataValues = $(data).map(function(i,d){
				var maxdata = d.value;
				if (d.comparison){
...
*/
			defaultMax = !valueispercentage ? d3.max(maxDataValues, function(d) { return d }) : 100;

		var valueColorDefaultScale = d3.scaleLinear()
				.domain([0, defaultMax])
				.range([value_minColor, value_maxColor]),
			comparisonColorDefaultScale = d3.scaleLinear()
				.domain([0, defaultMax])
				.range([comparison_minColor, comparison_maxColor]);
		...
		data = $(data).map(function(i, d) {
			...
			if (minvalue || maxvalue) {
				...
				valueColorScale = d3.scaleLinear()
					.domain([minValue, maxValue])
					.range([value_minColor, value_maxColor]).nice();
				
				comparisonColorScale = d3.scaleLinear()
					.domain([minValue, maxValue])
					.range([comparison_minColor, comparison_maxColor]).nice();
		...
		var valueFormat = ib3SLI.config.getFormatByBucketName('value', 0),
			total = $(data)
				.filter(function(i,d) { return !valueispercentage; })
		...
		if (ib3SLI.config.getBucket('comparison')){
			var progress2 = groups.append('rect')
		...
		if (ib3SLI.config.getBucket('minpoint')){
			var pointMin = groups.append('rect')
		...
		if (ib3SLI.config.getBucket('maxpoint')){
			var pointMax = groups.append('rect')
...
/*
		var legend = groups.append('text')
			.attr('fill','black')
			.attr('y', (rowHeight * maxDataValues.length) + 20)
			.attr('x', function(d, i) {
				return width / 4 * i;
			})
			.text(function(d){
