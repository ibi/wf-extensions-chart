	...
	function renderCallback(renderConfig) {
		...
		if (typeof data[0].measure !== 'undefined'||typeof data[0].dimension !== 'undefined') {
			...
			if (props.measureNumber.hideMeasureNumber == true) {
				...
				$(container).css('top',titleHeight+'px').append('<div class="top10-container-wo-measure" style="font-family:'+props.fontFamily+';font-size:'+props.fontSize+';color:'+props.color+'"></div>');
				if (props.headline.disableHeadline == true) {
				}
				else {
					// fill containers on html page - headline
					$(container).find('.top10-container-wo-measure').append('<div><div class="top10-headline" style="font-weight:'+props.headline.fontWeight+';font-size:'+props.headline.fontSize+';color:'+props.headline.color+'">' + title + '</div></div>');
				}
			}
			else {
				// create top10-container				
				$(container).css('top',titleHeight+'px').append('<div class="top10-container" style="font-family:'+props.fontFamily+';font-size:'+props.fontSize+';color:'+props.color+'"></div>');
				if (props.headline.disableHeadline == true) {
				}
				else {
					// fill containers on html page - headline
					$(container).find('.top10-container').append('<div><div class="top10-headline" style="font-weight:'+props.headline.fontWeight+';font-size:'+props.headline.fontSize+';color:'+props.headline.color+'">' + title + '</div></div>');
				}
			}
			...
			for (var z=0; z<props.numberOfTopEntries; z++) {
				if ( z < 10 ) {
					...
					if (props.measureNumber.hideMeasureNumber == true) {
						...
						$(container).find('.top10-container-wo-measure').append('<div><div class="top10-dimension-wo-measure" style="font-weight:'+props.dimensionText.fontWeight+';font-size:'+props.dimensionText.fontSize+';color:'+props.dimensionText.color+';text-align:'+props.dimensionText.textAlign+'">' + position + dim_text[z] + '</div></div>');
					}
					else {
						// fill containers on html page - dimension + measure
						$(container).find('.top10-container').append('<div><div class="top10-dimension" style="font-weight:'+props.dimensionText.fontWeight+';font-size:'+props.dimensionText.fontSize+';color:'+props.dimensionText.color+';text-align:'+props.dimensionText.textAlign+'">' + position + dim_text[z] + '</div><div class="top10-measure" style="font-weight:'+props.measureNumber.fontWeight+';font-size:'+props.measureNumber.fontSize+';color:'+props.measureNumber.color+'">' + msr_value[z] + percent + '</div></div>');						
					}
				}
			}	
		}
