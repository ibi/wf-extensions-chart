    ...
    return function (user_props) {
        ...
        function renderCircles(circles_group, onRenderComplete) {
            ...
            function buildAndPositionLabel(d) {
                var text = d3.select(this),
                    labels = jsonCopy((Array.isArray(d.data.labels)) ? d.data.labels : [d.data.labels]);
            ...
            if (dataHelper.isDynamicSize) {
                ...
                scale = (extent[0] * extent[1] < 0) ? d3.scaleLinear().domain(extent).range([0.01, Math.abs(extent[1] - extent[0])]) : d3.scaleLinear().domain(extent).range(extent);
            }
            ...
            var pack_data = pack(root).children.filter(function (d) {
                return d.r > 0;
            });
            ...
            node.append('circle')
                .attr('class', function (d) {
                    return d.data.elClassName;
                })
                .attr('r', function (d) {
                    return d.r;
                })
                .styles({
                    fill: colorMap ? function (d) { return colorMap[d.data.color]; } : '#00c9d3'
                });
        ...
        function renderLegend(legends_group, width, height, colorMap, title, widthRatio, props) {
            ...
            var x = width - legendHorizSpace,
             ...
             legends_group.selectAll('g.color-legend')
                .data([{}])
                .enter().append('g')
