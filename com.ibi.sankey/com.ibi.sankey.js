...
(function () {
    ...
    function renderCallback(renderConfig) {
	   ...
	   */
        ...
        var filteredData = renderConfig.data.filter(function (row) {
            //Filter out all Sources with 0 (zero) target values
            return row.value != 0;
        });
...
*/
        ...
        if (props.colors) {
            ...
            if (isJson(props.colors)) {
                Object.keys(colors).forEach(function (col) {
                    colors[col.toLowerCase()] = colors[col];
                });
            }
        }
        ...
        var sankey = d3.sankey().nodeWidth(nodeWidth).nodePadding(nodePadding).size([width, height]).nodes(data.nodes).links(data.links).align(sankeyAlign).layout(32);
        //		var sankey = d3.sankey().nodeWidth(nodeWidth).nodePadding(nodePadding).size([width, height]).nodes(data.nodes).links(data.links).nodeAlign(sankeyAlign).iterations(32).nodeSort(null);
        ...
        var link = container
            ...
            .attr('class', function (d, i) {
                var auxD = Object.assign({}, d);
                auxD.source = auxD.source.name;
                auxD.target = auxD.target.name;
                renderConfig.modules.tooltip.addDefaultToolTipContent(this, 0, i, auxD);
                //                return 'link ' + chart.buildClassName('riser', 0, i, 'bar');
                return chart.buildClassName('riser', 0, i, 'bar') + ' link';
            })
            //		   .attr("class", "link")
...
*/
            ...
            .style('stroke', function (link) {
                var clrSource, clrTarget, auxSourceName, auxTargetName;
                //                auxSourceName = link.source.name.replace(/ .*/, '');
                //                auxTargetName = link.target.name.replace(/ .*/, '');
                auxSourceName = link.source.name.trim();
                auxTargetName = link.target.name.trim();
                if (isJson(props.colors)) {
                    clrSource = colors[auxSourceName.toLowerCase()];
                    if (!clrSource) {
                        Object.keys(colors).forEach(function (col) {
                            if (auxSourceName.toLowerCase().indexOf(col) != -1) clrSource = colors[col];
                        });
                    }
                    clrTarget = colors[auxTargetName.toLowerCase()];
                    if (!clrTarget) {
                        Object.keys(colors).forEach(function (col) {
                            if (auxTargetName.toLowerCase().indexOf(col) != -1) clrTarget = colors[col];
                        });
                    }
                } else if (Array.isArray(props.colors)) {
                    clrSource = colors[link.source.id];
                    clrTarget = colors[link.target.id];
                } else {
                    clrSource = color(auxSourceName);
                    clrTarget = color(auxTargetName);
                }

                var gradient = svgDefs
                ...
                var fillColor1 = props.colorMode ? clrSource : null;
                var fillColor2 = props.colorMode ? clrTarget : null;
                var fillColor3 = 'url(#grad_' + link.source.id + '_' + link.target.id + ')';
                return props.colorMode.toLowerCase() === 'source' ? fillColor1 : props.colorMode.toLowerCase() === 'target' ? fillColor2 : props.colorMode.toLowerCase() === 'gradient' ? fillColor3 : props.linkColor;
            })
        ...
        node.append('rect')
            ...
            .attr('tdgtitle', function (d) {
                return d.name + ': <b>' + formatNumber(d.value) + '</b>';
                //                return "<span style='color: WhiteSmoke; background-color:rgba(0,0,0,.8);'>" + d.name + ': ' + formatNumber(d.value) + '</span>';
            })
            .attr('shape-rendering', 'crispEdges')
            .attr('stroke-width', 1)
            .attr('fill-opacity', 0.9)
            .style('fill', function (d) {
                var fillColor, auxName;
                //                auxName = d.name.replace(/ .*/, '');
                auxName = d.name.trim();
                if (isJson(props.colors)) {
                    fillColor = colors[auxName.toLowerCase()];
                    if (!fillColor) {
                        Object.keys(colors).forEach(function (col) {
                            if (auxName.toLowerCase().indexOf(col) != -1) fillColor = colors[col];
                        });
                    }
                } else if (Array.isArray(props.colors)) {
                    fillColor = colors[d.id];
                } else {
                    fillColor = color(auxName);
                }
                return fillColor;
            })
        ...
        function nodemouseover(_) {
            ...
            var allNodes = d3.selectAll('.node').attr('opacity', function (d) {
                var a = objThis === d.name || objSource.indexOf(d.name) >= 0 || objTarget.indexOf(d.name) >= 0 ? '1' : props.fadeOpacity;
                return a;
            });
            var allNodes = d3.selectAll('.link').attr('stroke-opacity', function (d) {
                var a = objThis === d.source.name || objThis === d.target.name ? '0.5' : props.fadeOpacity;
                return a;
            });
        }

        function linkmouseover(_) {
            var objTarget = _.source.name,
                objSource = _.target.name;
            var allNodes = d3.selectAll('.node').attr('opacity', function (d) {
                var a = objSource === d.name || objTarget === d.name ? '1' : props.fadeOpacity;
                return a;
            });
            var allNodes = d3.selectAll('.link').attr('stroke-opacity', function (d) {
                var a = objTarget === d.source.name && objSource === d.target.name ? '0.5' : props.fadeOpacity;
        ...
        d3.selectAll('path').attr('class', function () {
            return this.getAttribute('class') + ' fill';
        });
    ...
    var _arrScripts = window.d3 ? ['lib/sankey.js'] : ['lib/d3.v5.16.min.js'];
    _arrScripts = _arrScripts.concat(['lib/d3-sankey.js', 'lib/d3-selection-multi.min.js', 'lib/d3-time.min.js', 'lib/d3-time-format.min.js', 'lib/d3-transition.min.js']);
    //	_arrScripts = _arrScripts.concat(['lib/d3-array.min.js','lib/d3-collection.min.js','lib/d3-path.min.js','lib/d3-shape.min.js','lib/d3-sankey.min.js']);
