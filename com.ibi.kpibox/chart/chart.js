(function() {
	...
	function _draw(isDummyData) {
		...
		if(shortenNumbers) {
			var abbr = $ib3.utils.getNumericAbbreviation(kpiValue),
				shortenNumber = $ib3.utils.getShortenNumberByAbbreviation(kpiValue, abbr),
			...
			if(lastCharValueFormat == '%') {
				valueFormatApplied = valueFormatApplied.replace('`%', '%').replace('%', abbr + lastCharValueFormat);								
			...
			kpiValue = $ib3.config.formatNumber(shortenNumber, valueFormatApplied); 
		} else {
			kpiValue = $ib3.config.formatNumber(kpiDataElem.value, numberFormat); 	
		}
