    ...
    function _draw(ib3SLI, isDummyData) {
        ...
        function generateComparation(parameters) {
            var field = parameters.field,
                ...
                kpiValue = parameters.kpiValue || 0,
                //				value = parameters.kpiValue,
                compareValue = parameters.compareValue || 0,
            ...
            function _setValueCompare() {
                $templateComparation2.find('.kpiBoxCompareValue').text(percentValueFormater).css('color', _colorComparation);
                $(container)
                    .closest('body')
                    .css({
            ...
            function _setValueReal() {
                ...
                var _compareValue = (kpiFormattedValue = $ib3.utils.getFormattedNumber(ib3SLI.config.formatNumber, compareValue, auxNumberFormat, shortenNumber, typeShortenNumber));
                ...
                $('.kpiBoxValueCompareRow', window.comIbiKpiboxChartExtension.container)
                    .css({
                        'font-size':
                    ...
                    .append($('<div>', window.comIbiKpiboxChartExtension.container).text(_compareValue));
            ...
            function _setDefaultIcon() {
                ...
                $templateComparation2.find('.kpiBoxCompareImage').addClass(_iconDefault).css('border-color', _borderColor).css('zoom', zoomIcon);
            }
            ...
            function _formattedPercentage(percentageCalcValue) {
                ...
                if (setInfiniteToZero) {
                    percentageFormattedValue = percentageCalcValue == 'Infinity' || percentageCalcValue == '-Infinity' || isNaN(percentageCalcValue) ? ib3SLI.config.formatNumber(0, formatComparation) : ib3SLI.config.formatNumber(parseFloat(percentageCalcValue).toFixed(4), formatComparation);
                } else {
        ...
        d3.select('.chartPanel').attr('class', ib3SLI.config.getDrillClass('riser', 0, 0)).append('rect').attr('class', 'drill').attr('width', d3.select('.eventCatcher').attr('width')).attr('height', d3.select('.eventCatcher').attr('height'));
