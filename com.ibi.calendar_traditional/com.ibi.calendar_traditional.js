...
(function () {
	...
	var utils = {
		...
		checkDateFormat: function (sampleDateString, expectedDateFormat) {
			var regexYMD = /^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])$/;
			var regexDMY = /^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/;
			var regexMDY = /^(0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])[\/\-]\d{4}$/;

			var formatType = 'YMD';

			formatType = regexYMD.test(sampleDateString) ? 'YMD' : regexMDY.test(sampleDateString) ? 'MDY' : 'DMY';

			var toCompareFormat = expectedDateFormat.toLowerCase();

			if(toCompareFormat == "%d/%m/%y" || toCompareFormat == "%d-%m-%y" || toCompareFormat == "%d.%m.%y")
			{
				return formatType == 'DMY';
			}

			if(toCompareFormat == "%m/%d/%y" || toCompareFormat == "%m-%d-%y" || toCompareFormat == "%m.%d.%y")
			{
				return formatType == 'MDY';
			}

			if(toCompareFormat == "%y/%m/%d" || toCompareFormat == "%y-%m-%d" || toCompareFormat == "%y.%d.%d")
			{
		...
		labels:{
			en: {
				...
				,errorMsgLine2: 'Change your date field format or change the extension\'s date format property.'
			}
		}
	}
	//end VIZ-391
	...
	function renderCallback(renderConfig) {
		...
		var testDate = utils.checkDateFormat(data[0].dimension,  renderConfig.properties.calendarConfig.dateFormat);

		if(!testDate)
		{
			//if not, then show an error message
			var messageHTML = [];
			messageHTML.push('<div id="'+renderConfig.rootContainer.id+'-messageContainer">');
			messageHTML.push('<div style="display: table-cell; vertical-align: middle; text-align: center;">');
			messageHTML.push(utils.labels.en.errorMsgLine1+ renderConfig.properties.calendarConfig.dateFormat+"<br/>");
			...
			var msgContainer = $('#'+renderConfig.rootContainer.id+'-messageContainer');
