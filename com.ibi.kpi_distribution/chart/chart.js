(function () {
    ...
    function _draw(ib3SLI, isDummyData) {
        ...
        var _cssUrl = ib3SLI.config.getProperty('kpidistributionProperties.cssUrl') || null;

        if (_cssUrl != null) {
            $.when($.getStylesheet(_cssUrl)).done(function () {
                _drawNow(ib3SLI, isDummyData);
            });
        } else {
            _drawNow(ib3SLI, isDummyData);
        }
    ...
    function _drawNow(ib3SLI, isDummyData) {
        ...
        var barWidth = width,
            ...
            showBarIcons = data[0].image ? true : false,
        ...
        if (showBarIcons) barWidth = barWidth - barIconWidth;
        ...
        var defaultMin = 0,
            ...
            allValues = $(data)
                ...
                .get(),
            defaultMax = nonPercentageValues.length
                ? nonPercentageValues.reduce(function (total, elem) {
                      return total + elem;
                  }, 0)
                : 100;

        var colorDefaultScale = d3.scaleLinear().domain([0, defaultMax]).range([minColor, maxColor]);
        ...
        data = $(data).map(function (i, d) {
            ...
            if (hasMinValue || hasMaxValue) {
                widthScale = d3.scaleLinear().domain([minValue, maxValue]).range([0, barWidth]);

                colorScale = d3.scaleLinear().domain([minValue, maxValue]).range([minColor, maxColor]);
            }
