...
(function () {
  ...
  function renderCallback(renderConfig) {
    ...
    var max_value = data[0].max == undefined ? 100 : data[0].max;
    /*END VIZ-235 - fixed handling no min / max set */
    ...
    var chart = c3.generate({
      ...
      gauge: {
        label: {
          format: function (value, ratio) {
            /*BEGIN VIZ-235 - format string using tdg helper function 
				renderConfig.moonbeamInstance.parseTemplate(
				k = (string) {{extension_bucket("geo_dimension",0)}} {{extension_bucket("measures",0)|#("$#,###")}}
				,g = (object) the data point from the dataset
				,c = (array) the entire dataset
				,q = (not sure what this is used for)
				)			
				NOTE: the value for k comes from renderConfig.moonbeamInstance.getSeries(0).tooltip.WF_fields
			*/

            var tooltip = renderConfig.moonbeamInstance.getSeries(0).tooltip;

            //check to see if there is a tooltip defined
            if (tooltip != undefined) {
              //if the below is true, this is either IA with API 1.0 or Designer
              if (
                renderConfig.moonbeamInstance.getSeries(0).tooltip.WF_fields ==
                undefined
              ) {
                var tooltip_content = renderConfig.moonbeamInstance.getSeries(0)
                  .tooltip;
                var formatting = "";

                //renderConfig.dataBuckets.internal_api_version
                //search the content for the actual string
            ...
            /*END VIZ-235 - format string using tdg helper function */
      ...
      onrendered: function () {
        if (target != null) {
          ...
          var target_degrees =
            (relative_target * 180) / full_scale + degree_offset;
    ...
    if (renderConfig.properties.show_legend) {
      ...
      if (color_states.length > 1) {
        ...
        for (var index = 0; index < color_states.length; index++) {
          legend_html.push("<td>");
          legend_html.push(
            '<div style="width: 12px; height: 12px; display: inline-block; background-color: ' +
              color_states[index] +
              '"></div>'
          );
          legend_html.push("</td>");
        ...
        $(".com_ibi_gauge_table").css(
          "margin-left",
          "calc((100% - " + table_width + "px)/2)"
        );
      }
    }
